...
export function fetchProducts() {
    if (productsWithPrice.length === 0) {
        ...
        productsWithPrice = products.map((product) => {
            const newProduct = {
                ...product,
                price: Math.round(Math.random() * 90000) / 100 + 10,
                thumbnailUrl: `https://picsum.photos/150?random=${product.id}`,
                url: `https://picsum.photos/600?random=${product.id}`
            }
            delete newProduct.albumId
            return newProduct
        })
    }
